...
// ============================================================
//  Gutachter-Modus: Fragestellung + gespeicherter Fall (Vorgang)
// ============================================================

/// <summary>Fragestellung (Anlass) einer MPU-Begutachtung.</summary>
public enum Fragestellung
{
    [Description("Alkohol")] Alkohol,
    [Description("Drogen")] Drogen,
    [Description("Verkehrsauffälligkeiten")] Verkehrsauffaelligkeiten,
    [Description("Straftaten")] Straftaten,
    [Description("Medizinische Fragestellung")] Medizinisch,
    [Description("Kombination mehrerer Fragestellungen")] Kombination
}

/// <summary>
/// Ein Gutachter-Vorgang: ein gespeicherter Fall einer MPU-Simulation. Wird als eigener Ordner
/// unter AppData abgelegt und kann spaeter wieder geoeffnet werden. Felder fuer Auffaelligkeitsliste
/// und Fragen sind bereits vorbereitet (werden in den naechsten Ausbaustufen befuellt).
/// </summary>
public class GutachterCase
{
    public string Id { get; set; } = Guid.NewGuid().ToString("N");

    public string FirstName { get; set; } = string.Empty;
    public string LastName { get; set; } = string.Empty;
    public string BirthDate { get; set; } = string.Empty;   // freie Eingabe (TT.MM.JJJJ)
    public string Language { get; set; } = "Deutsch";
    public int QuestionCount { get; set; } = 15;
    public Fragestellung Fragestellung { get; set; } = Fragestellung.Alkohol;
    public string ExamDate { get; set; } = string.Empty;    // Pruefungsdatum (frei)
    public string Notes { get; set; } = string.Empty;

    // Vorbereitet fuer spaetere Ausbaustufen (Auffaelligkeitsanalyse, Fragenkatalog, Pruefung):
    public string AuffaelligkeitenText { get; set; } = string.Empty;
    public string AuffaelligkeitenAnalyse { get; set; } = string.Empty;
    public List<TrainingQuestion> Questions { get; set; } = new();
    public string Gesamtbewertung { get; set; } = string.Empty;

    public DateTime CreatedAt { get; set; } = DateTime.Now;
    public DateTime UpdatedAt { get; set; } = DateTime.Now;

    /// <summary>Vollstaendiger Name fuer Anzeigen.</summary>
    public string FullName => $"{FirstName} {LastName}".Trim();

    /// <summary>Kurze Anzeige fuer Listen (Name, Fragestellung, Datum).</summary>
    public string DisplayLine
    {
        get
        {
            var name = string.IsNullOrWhiteSpace(FullName) ? "(ohne Namen)" : FullName;
            var datum = string.IsNullOrWhiteSpace(ExamDate) ? CreatedAt.ToString("dd.MM.yyyy") : ExamDate;
            return $"{name}  ·  {EnumText(Fragestellung)}  ·  {datum}";
        }
    }
    ...
    private static string EnumText(Enum value)
    {
        ...
        return attr is { Length: > 0 } ? ((DescriptionAttribute)attr[0]).Description : value.ToString();
    }
}
